...
namespace BLL
{
    public class BLL_Water_processLotArray
    {
        public List<Model_Water_processLotArray> GetAllContacts()
        {
            lock (GlobalsInfo.Global_Global.thisLock_xml)
            {
                XDocument xmlDocument = XDocument.Load(AddressFile_Mes.dbPath + "\\Model_Water_processLotArray.xml");//Contacts

                IEnumerable<XElement> xmlContacts = xmlDocument.Element("Models")
                    .Elements("Model");

                List<Model_Water_processLotArray> contacts = new List<Model_Water_processLotArray>();

                //获取列表
                foreach (XElement i in xmlContacts)
                {
                    Model_Water_processLotArray contact = new Model_Water_processLotArray();
        ...
        public Model_Water_processLotArray GetContactById(int id)
        {
            lock (GlobalsInfo.Global_Global.thisLock_xml)
            {
                XDocument xmlDocument = XDocument.Load(AddressFile_Mes.dbPath + "\\Model_Water_processLotArray.xml");
        ...
        public void AddContact(Model_Water_processLotArray contact)
        {
            lock (GlobalsInfo.Global_Global.thisLock_xml)
            {
                XDocument xmlDocument = XDocument.Load(AddressFile_Mes.dbPath + "\\Model_Water_processLotArray.xml");
                ...
                xmlDocument.Save(AddressFile_Mes.dbPath + "\\Model_Water_processLotArray.xml");
            }
        }

        public void UpdateContact(Model_Water_processLotArray contact)
        {
            lock (GlobalsInfo.Global_Global.thisLock_xml)
            {
                XDocument xmlDocument = XDocument.Load(AddressFile_Mes.dbPath + "\\Model_Water_processLotArray.xml");
        ...
        public void SetXmlFile()
        {
            ...
            {
                //如果xml文件丢失，则使用根元素创建它
                if (!(File.Exists(AddressFile_Mes.dbPath + "\\Model_Water_processLotArray.xml")))
                {
