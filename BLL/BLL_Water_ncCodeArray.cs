...
namespace BLL
{
    public class BLL_Water_ncCodeArray
    {
        public List<Model_Water_ncCodeArray> GetAllContacts()
        {
            lock (GlobalsInfo.Global_Global.thisLock_xml)
            {
                XDocument xmlDocument = XDocument.Load(AddressFile_Mes.dbPath + "\\Model_Water_ncCodeArray.xml");

                IEnumerable<XElement> xmlContacts = xmlDocument.Element("Models")
                    .Elements("Model");

                List<Model_Water_ncCodeArray> contacts = new List<Model_Water_ncCodeArray>();

                //获取 列表
                foreach (XElement i in xmlContacts)
                {
                    Model_Water_ncCodeArray contact = new Model_Water_ncCodeArray();
        ...
        public Model_Water_ncCodeArray GetContactById(int id)
        {
            lock (GlobalsInfo.Global_Global.thisLock_xml)
            {
                XDocument xmlDocument = XDocument.Load(AddressFile_Mes.dbPath + "\\Model_Water_ncCodeArray.xml");
        ...
        public void AddContact(Model_Water_ncCodeArray contact)
        {
            lock (GlobalsInfo.Global_Global.thisLock_xml)
            {
                XDocument xmlDocument = XDocument.Load(AddressFile_Mes.dbPath + "\\Model_Water_ncCodeArray.xml");
                ...
                xmlDocument.Save(AddressFile_Mes.dbPath + "\\Model_Water_ncCodeArray.xml");
            }
        }

        public void UpdateContact(Model_Water_ncCodeArray contact)
        {
            lock (GlobalsInfo.Global_Global.thisLock_xml)
            {
                XDocument xmlDocument = XDocument.Load(AddressFile_Mes.dbPath + "\\Model_Water_ncCodeArray.xml");
        ...
        public void SetXmlFile()
        {
            ...
            {
                //如果xml文件丢失，则使用根元素创建它
                if (!(File.Exists(AddressFile_Mes.dbPath + "\\Model_Water_ncCodeArray.xml")))
                {
