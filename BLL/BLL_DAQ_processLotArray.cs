...
namespace BLL
{
    public class BLL_DAQ_processLotArray  // 数据采集
    {
        public List<Model_DAQ_processLotArray> GetAllContacts()
        {
            lock (GlobalsInfo.Global_Global.thisLock_xml)
            {
                XDocument xmlDocument = XDocument.Load(AddressFile_Mes.dbPath + "\\Model_DAQ_processLotArray.xml");//Contacts

                IEnumerable<XElement> xmlContacts = xmlDocument.Element("Models")
                    .Elements("Model");

                List<Model_DAQ_processLotArray> contacts = new List<Model_DAQ_processLotArray>();

                //获取列表
                foreach (XElement i in xmlContacts)
                {
                    Model_DAQ_processLotArray contact = new Model_DAQ_processLotArray();
        ...
        public Model_DAQ_processLotArray GetContactById(int id)
        {
            lock (GlobalsInfo.Global_Global.thisLock_xml)
            {
                XDocument xmlDocument = XDocument.Load(AddressFile_Mes.dbPath + "\\Model_DAQ_processLotArray.xml");
        ...
        public void AddContact(Model_DAQ_processLotArray contact)
        {
            lock (GlobalsInfo.Global_Global.thisLock_xml)
            {
                XDocument xmlDocument = XDocument.Load(AddressFile_Mes.dbPath + "\\Model_DAQ_processLotArray.xml");
                ...
                xmlDocument.Save(AddressFile_Mes.dbPath + "\\Model_DAQ_processLotArray.xml");
            }
        }

        public void UpdateContact(Model_DAQ_processLotArray contact)
        {
            lock (GlobalsInfo.Global_Global.thisLock_xml)
            {
                XDocument xmlDocument = XDocument.Load(AddressFile_Mes.dbPath + "\\Model_DAQ_processLotArray.xml");
        ...
        public void SetXmlFile()
        {
            ...
            {
                //如果xml文件丢失，则使用根元素创建它
                if (!(File.Exists(AddressFile_Mes.dbPath + "\\Model_DAQ_processLotArray.xml")))
                {
